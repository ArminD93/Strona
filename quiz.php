<?php
//Connect with mysql
$db = new mysqli("localhost", "root", "", "zagle_pytania");
//Perform the query to choose random questions
$query = $db->query("SELECT `id`, `tresc`, `OdpA`, `OdpB`, `OdpC` FROM `meteo` ORDER BY RAND() LIMIT 20");
...
session_start();
if (isset($_GET['tresc'])) {
    $question = preg_replace('/[^0-9]/', "", $_GET['tresc']);
    $next     = $question + 1;
    $prev     = $question - 1;
...
?>
    ...
    document.getElementById('tresc').innerHTML = nums[0];
